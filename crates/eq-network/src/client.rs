...
//!
//! ```no_run
//! use eq_network::{chat::OutboundChat, client::{CancellationToken,
...
/// Typed work submitted by the host while a character is connected.
...
/// Login secrets retained only for the lifetime of a client configuration.
#[derive(Clone)]
...
impl Credentials {
    /// Store an account name and password with zeroization on drop.
    ...
    /// Return the account name for packet encoding.
    #[must_use]
...
/// Connection and logging preferences.
#[derive(Clone, Debug)]
#[non_exhaustive]
pub struct ClientConfig {
    /// Server family and protocol generation.
    pub protocol: ServerProtocol,
    /// Login server hostname or address.
    pub host: String,
    /// Login server UDP port.
    pub port: u16,
...
impl ClientConfig {
    ...
    fn validate(&self) -> Result<()> {
        ...
        for value in [self.credentials.account(), self.credentials.password()] {
                ...
                !value.is_empty() && !value.contains('\0'),
                "required credentials must be nonempty and contain no NUL"
            );
        }
...
pub struct ClientIdentity {
    /// Short device hostname announced during P99 validation.
    pub hostname: String,
    /// Short operating-system username announced during P99 validation.
    pub username: String,
}

impl ClientIdentity {
    /// Construct host metadata for the P99 validation packet.
...
pub struct RunOptions {
    /// Reconnect after transient session failures.
    pub reconnect: bool,
    /// Stop after character selection without opening the zone connection.
    pub world_only: bool,
...
/// Coarse connection state for health checks and user interfaces.
#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "snake_case")]
#[non_exhaustive]
pub enum ConnectionState {
    /// A fresh login attempt is in progress.
    Connecting,
    /// The character is active in a zone.
    Connected,
    /// A zone handoff is in progress.
    Zoning,
    /// The current network session ended.
    Disconnected,
    /// The client stopped after cancellation or a non-reconnecting run.
    Stopped,
}
...
pub struct SessionStatus {
    ...
    /// Unix timestamp when the status was emitted.
    pub timestamp: i64,
    /// Unique identifier for the current process-level session.
    pub session_id: String,
    /// Current short zone name, if known.
    pub zone: String,
    /// Number of communication records received in this attempt.
    pub messages: u64,
    /// Number of application packets received in this attempt.
    pub packets: u64,
    /// Seconds since the last valid server datagram, if a session is active.
...
pub struct DecodeError {
    #[serde(rename = "type")]
    kind: &'static str,
    /// Raw application opcode.
    pub opcode: u16,
    /// Hex-encoded packet body retained for diagnosis.
...
pub enum RecordEvent {
    /// Successfully decoded communication.
    Chat(ChatEvent),
    /// Communication packet that failed validation or decoding.
...
pub struct Record {
    ...
    /// Selected world-server display name.
    pub server: String,
    /// Logged-in character name.
    pub character: String,
    /// Current short zone name.
    pub zone: String,
    /// Process-level session identifier.
    pub session_id: String,
    /// Monotonic record number within the session.
    pub message_id: u64,
...
pub enum ConnectionStage {
    /// Opening the login-server transport.
    ConnectingLogin,
    ...
    ConnectingWorld,
    /// Requesting and selecting the configured character.
    SelectingCharacter,
    /// Opening the assigned zone-server transport.
    ConnectingZone,
    /// Receiving character and zone initialization data.
    LoadingCharacter,
    /// Completing final zone admission acknowledgements.
    EnteringWorld,
    /// Character is active and chat commands can be sent.
    Ready,
}
...
pub enum ClientEvent {
    ...
    /// Decoded communication record.
    Record(Box<Record>),
    /// Credential-safe operational diagnostic.
    Diagnostic(String),
    /// Transient failure followed by a scheduled fresh session.
    Reconnecting {
        /// Credential-safe failure text.
        error: String,
...
impl Client {
    ...
    /// # Errors
    ///
    /// Returns an error when configuration, identity metadata, or the bundled
    ...
    /// # Errors
    ///
    /// Returns an error when the supplied inventory lacks required spell data.
    ...
    /// # Errors
    ///
    /// Returns a [`LoginError`] for rejected credentials and otherwise returns
    /// transport, protocol, cancellation, or event-handler errors.
    pub fn run(
        &self,
    ...
    ///
    /// Returns a [`LoginError`] for rejected credentials and otherwise returns
    /// transport, protocol, cancellation, command, or event-handler errors.
    ...
    fn run_inner(
        ...
        loop {
            ...
            if result
                .as_ref()
                .is_err_and(anyhow::Error::is::<EventDeliveryError>)
            {
                return result;
            }
            ...
            if !options.reconnect || result.as_ref().is_err_and(anyhow::Error::is::<LoginError>) {
                return result;
            }
