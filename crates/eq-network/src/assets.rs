...
/// Measured CRC-32 and size for one client file.
#[derive(Debug, Serialize, Deserialize)]
pub struct FileChecksum {
    /// CRC-32 expected by server-side file validation.
    pub crc32: u32,
    /// File length in bytes.
    pub size: u64,
}

/// Checksum inventory for one patched client installation.
#[derive(Debug, Serialize, Deserialize)]
pub struct Assets {
    /// Human-readable client profile represented by the inventory.
    pub client: String,
    /// Case-normalized filename to measured checksum, or `None` when explicitly unknown.
...
pub struct FileResponse {
    /// Encoded manifest hash and per-file CRC values.
    pub body: Vec<u8>,
    /// Requested filenames that were absent from the inventory.
...
impl Assets {
    ...
    /// # Errors
    ///
    /// Returns an error if the bundled inventory cannot be decoded.
    ...
    /// # Errors
    ///
    /// Returns an error when the installation cannot be read or a discovered
    ...
    /// # Errors
    ///
    /// Returns an error when a requested name is unsafe or a present file cannot
    /// be read.
    ...
    /// # Errors
    ///
    /// Returns an error when the inventory has no measured spell file.
...
/// One validated request from a V62 file manifest.
#[derive(Debug, Eq, PartialEq)]
pub struct ManifestEntry {
    /// Server-provided identifier echoed in the response.
    pub id: u16,
    /// Manifest behavior flags.
    pub flags: u8,
    /// Validated filename without path components.
    pub name: String,
}
...
/// # Errors
///
/// Returns an error for truncated entries, unsafe filenames, unsupported flags,
/// duplicate identifiers, or an empty manifest.
