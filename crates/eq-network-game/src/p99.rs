...
/// # Errors
///
/// Returns an error when the complete application length exceeds `u16`.
...
/// Stateful Titanium/P99-V62 codec for world approval and file validation.
...
impl WorldCodec {
    ...
    /// # Errors
    ///
    /// Returns an error unless `login_info` has the 464-byte Titanium layout.
    ...
    /// # Errors
    ///
    /// Returns an error when the challenge is malformed or contains unsupported
    /// public-key values.
    ...
    /// # Errors
    ///
    /// Returns an error unless `body` has the 272-byte approval layout.
    ...
    /// # Errors
    ///
    /// Returns an error when approval has not initialized the incoming state.
    ...
    /// # Errors
    ///
    /// Returns an error when no manifest has initialized the outgoing state.
    ...
    /// # Errors
    ///
    /// Returns an error when the handoff is too short or its digest input is too large.
    ...
    /// # Errors
    ///
    /// Returns an error unless `entry` has the 68-byte Titanium layout.
    ...
    /// # Errors
    ///
    /// Returns an error unless `body` has the 385-byte Titanium layout.
...
/// # Errors
///
/// Returns an error when the session key is empty.
