...
/// Stable names for communication channels emitted by the client.
...
pub enum ChannelName {
    ...
    /// Group chat.
    Group,
    /// Zone-wide shout chat.
    Shout,
    /// Zone-wide auction chat.
    #[serde(alias = "auc", alias = "/auc")]
    Auction,
    /// Zone-wide out-of-character chat.
    #[serde(alias = "/ooc")]
    Ooc,
    /// Server broadcast.
    Broadcast,
    ...
    Motd,
    /// Formatted or special system message.
    System,
    /// Guild message of the day.
    GuildMotd,
    /// Unrecognized channel identifier retained for forward compatibility.
    Unknown,
}
...
pub enum OutboundChat {
    ...
    Group(String),
    /// Send text to zone-wide shout chat.
    Shout(String),
    /// Send text to zone-wide auction chat.
    Auction(String),
    /// Send text to zone-wide out-of-character chat.
    Ooc(String),
    /// Send a direct tell.
    Tell {
    ...
    /// Send text to nearby characters.
    Say(String),
    /// Send text to raid chat.
    Raid(String),
}

impl OutboundChat {
    const fn channel(&self) -> u16 {
        match self {
            Self::Guild(_) => 0,
            Self::Group(_) => 2,
            Self::Shout(_) => 3,
...
/// by the selected client packet layout.
pub fn encode_outbound_for(
    protocol: GameDialect,
    ...
    let trailer = usize::from(protocol == GameDialect::EqMac) * 4;
    ...
    match protocol {
        ...
        GameDialect::EqMac => {
            body[130..132].copy_from_slice(&message.channel().to_le_bytes());
...
/// One item link extracted from a decoded chat message.
#[derive(Clone, Debug, Serialize)]
pub struct ItemLink {
    /// Exact link body between the wire delimiters.
    pub body: String,
    /// Human-readable item label.
    pub text: String,
    /// Inclusive/exclusive offsets in the original wire bytes, including delimiters.
    pub start: usize,
    /// Exclusive byte offset after the link's closing wire delimiter.
    pub end: usize,
    /// Inclusive/exclusive UTF-8 byte offsets of the label in `Message::text`.
    pub text_start: usize,
    /// Exclusive UTF-8 byte offset of the label in `Message::text`.
    pub text_end: usize,
    /// Numeric item identifier decoded for the selected client generation.
    pub item_id: u32,
}

/// Decoded display text with optional raw representation and structured links.
#[derive(Clone, Debug, Serialize)]
pub struct Message {
    /// Lossy decoded wire text when raw output was requested.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub message: Option<String>,
    /// Hex-encoded wire text when raw output was requested.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub message_hex: Option<String>,
    /// Readable display text with wire markup decoded.
    pub text: String,
    /// Structured links in display order.
...
pub struct ChatEvent {
    #[serde(rename = "type")]
    kind: &'static str,
    /// Raw game application opcode.
    pub opcode: u16,
    /// Hex-encoded packet body when raw output was requested.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub payload_hex: Option<String>,
    /// Normalized communication channel.
    pub channel_name: ChannelName,
    /// Decoded message when the packet carries display text.
    #[serde(flatten, skip_serializing_if = "Option::is_none")]
    pub message: Option<Message>,
    /// Original numeric channel identifier when present.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub channel: Option<u32>,
    /// Sending character or server name when present.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub sender: Option<String>,
    /// Receiving character or audience when present.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub target: Option<String>,
    /// String-table identifier for a formatted system message.
    #[serde(skip_serializing_if = "Option::is_none")]
...
fn message_for(protocol: GameDialect, bytes: &[u8], include_raw: bool) -> Message {
...
mod tests {
    ...
    fn quarm_outbound_chat_keeps_percent_text_and_adds_the_eqmac_trailer() {
        ...
        let body = encode_outbound_for(GameDialect::EqMac, &message, "ExampleCharacter").unwrap();
    ...
    fn quarm_communication_structs_use_eqmac_offsets() {
        ...
        let event = parse_for(GameDialect::EqMac, 0x3642, &formatted, false)
            .unwrap()
            .unwrap();
        ...
        let event = parse_for(GameDialect::EqMac, 0x8041, &special, false)
            .unwrap()
            .unwrap();
        ...
        let event = parse_for(GameDialect::EqMac, 0x0442, &motd, false)
            .unwrap()
            .unwrap();
