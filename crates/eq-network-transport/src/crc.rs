#[must_use]
/// Calculate the keyed CRC-32 variant used by the SOE transport.
...
#[must_use]
/// Append the negotiated two- or four-byte CRC representation to a packet.
pub fn append_crc(packet: &[u8], key: u32, crc_bytes: u8) -> Vec<u8> {
    ...
    if crc_bytes == 2 {
        out.extend_from_slice(&crc.to_be_bytes()[2..]);
    } else {
...
#[must_use]
/// Remove the negotiated CRC suffix without validating its value.
...
fn crc_table() -> &'static [u32; 256] {
    ...
    TABLE.get_or_init(|| {
        ...
        for (i, slot) in table.iter_mut().enumerate() {
            let mut crc = u32::try_from(i).expect("CRC table index fits in u32");
            for _ in 0..8 {
                if crc & 1 != 0 {
