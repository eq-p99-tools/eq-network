...
impl OldSession {
    ...
    /// # Errors
    ///
    /// Returns an error when cancellation was requested or the UDP socket
    /// cannot be bound, connected, or configured.
    pub fn connect_cancellable(address: SocketAddr, stop: &AtomicBool) -> Result<Self> {
    ...
    /// # Errors
    ///
    /// Returns an error when the session is closed, a protocol limit is
    /// exceeded, or the UDP socket cannot send a fragment.
    ...
    /// # Errors
    ///
    /// Returns an error for malformed datagrams, protocol-limit violations,
    ...
    fn process(&mut self, packet: Decoded) -> Result<()> {
        ...
        if !packet.selective_ack.is_empty() {
            ...
            for (byte_index, byte) in packet.selective_ack.iter().enumerate() {
                for bit in 0..8 {
                    let offset = u16::try_from(byte_index * 8 + bit)
                        .context("selective ACK offset exceeds sequence space")?;
                    let arq = start.wrapping_add(offset);
                    if byte & (0x80 >> bit) != 0 {
                        self.pending.retain(|pending| pending.arq != arq);
                    } else if let Some(pending) = self.pending.iter_mut().find(|p| p.arq == arq) {
                        pending.sent = Instant::now()
                            .checked_sub(Duration::from_secs(3))
                            .unwrap_or_else(Instant::now);
                    }
    ...
    /// # Errors
    ///
    /// Returns an error when the UDP socket cannot send the closing packet.
