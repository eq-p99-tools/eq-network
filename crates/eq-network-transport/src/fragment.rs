...
/// Bytes consumed by transport opcode, sequence, and total application length.
pub const FIRST_FRAG_OVERHEAD: usize = 8;
/// Bytes consumed by transport opcode and sequence on continuation fragments.
pub const SUBSEQUENT_FRAG_OVERHEAD: usize = 4;

/// Reassembles possibly reordered SOE fragments by their wrapping sequence number.
...
impl FragmentAssembler {
    #[must_use]
    /// Return whether an incomplete application is being assembled.
    pub fn is_active(&self) -> bool {
        self.first_seq.is_some()
    }

    /// Discard every buffered fragment and sequence marker.
    ...
    /// Add one raw fragment and return the application after all contiguous
    /// bytes have arrived.
...
#[must_use]
/// Read the transport opcode, total length, and application opcode from a first
/// fragment header.
...
/// Split an application payload across reliable transport fragments.
///
/// # Errors
///
/// Returns an error when the payload length exceeds the wire field or when
/// `max_packet` cannot hold a fragment header and data byte.
pub fn build_fragments(
    ...
    let wire_len = u32::try_from(total_len)
        .map_err(|_| ProtocolError::ApplicationTooLong { len: total_len })?;
    ...
    if first_capacity == 0 || subsequent_capacity == 0 {
        return Err(ProtocolError::PacketSizeTooSmall { size: max_packet });
    }
    ...
    hdr.extend_from_slice(&wire_len.to_be_bytes());
...
mod tests {
    ...
    fn duplicate_fragments_do_not_complete_with_gaps() {
        let payload = payload();
        let fragments = build_fragments(&payload, 10, 16).unwrap();
    ...
    fn duplicate_first_fragment_does_not_include_total_length() {
        let payload = payload();
        let fragments = build_fragments(&payload, 20, 16).unwrap();
        let mut assembler = FragmentAssembler::default();
    ...
    fn reassembles_contiguous_fragments_across_sequence_wrap() {
        let payload = payload();
        let fragments = build_fragments(&payload, u16::MAX - 1, 16).unwrap();
        let mut assembler = FragmentAssembler::default();
