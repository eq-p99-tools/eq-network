...
pub struct Application {
    /// Raw little-endian application opcode.
    pub opcode: u16,
    /// Reassembled application body without its opcode.
    pub body: Vec<u8>,
}
...
impl Session {
    ...
    /// # Errors
    ///
    /// Returns an error when the operating system cannot read the socket address.
    ...
    /// # Errors
    ///
    /// Returns an error when socket setup, negotiation, or validation fails.
    ...
    /// # Errors
    ///
    /// Returns an error on cancellation, timeout, socket failure, or an
    /// unsupported negotiation response.
    ...
    /// # Errors
    ///
    /// Returns an error when the session is closed, a size limit is exceeded,
    /// or the UDP socket cannot send a packet.
    ...
    /// # Errors
    ///
    /// Returns an error for an unsupported opcode, an oversized datagram, or a
    /// UDP socket failure.
    ...
    /// # Errors
    ///
    /// Returns an error for malformed packets, protocol-limit violations,
    /// timeouts, or UDP socket failures.
    ...
    // Keeping the opcode dispatch linear makes the session state transitions auditable.
    ...
    /// # Errors
    ///
    /// Returns an error when the UDP socket cannot send the disconnect packet.
